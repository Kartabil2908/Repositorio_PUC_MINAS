...
int main() { //Q03
    ...
    // Leitura da primeira parte (pilha inicial)
    while (1) {
        scanf("%s", entrada);
...
void inserirPokemonFila(FilaCircular* fila, Pokemon Pokemon)
{
    // verificando condição de inserção

    if(((fila->ultimo + 1) % 5) == fila->primeiro)
    {
        printf("Erro ao inserir na fila");
    }else
    {
        fila->pokemonsFila[fila->ultimo] = Pokemon;
