...
/* ============================ FUNÇÕES PARA MANIPULAÇÃO DE LISTA FlEXÍVEL SIMPLES ============================ */

void inicializarLista_populada(ListaFlexivelSimples **lista, Show *shows, int numShows) {
    ...
    *lista = NULL;
    ...
    for(int i = 0; i < numShows; i++) {
        ...
        if (novoNo == NULL) {
            perror("Falha ao alocar memória em inicializarLista_populada");
            exit(EXIT_FAILURE);
        }
        novoNo->show = shows[i];
        novoNo->prox = NULL;
        ...
        if(*lista == NULL) { 
            *lista = novoNo;
            ...
            ListaFlexivelSimples *aux = *lista;
            while(aux->prox != NULL) {
                aux = aux->prox;
            }
            aux->prox = novoNo;
...
void inserir_fim(ListaFlexivelSimples *lista, Show show)
{
    ListaFlexivelSimples *novoNo = (ListaFlexivelSimples *)malloc(sizeof(ListaFlexivelSimples));
    novoNo->show = show;
    novoNo->prox = NULL;
    ...
    if (lista == NULL) {
        lista = novoNo; // Se a lista estava vazia, agora aponta para o novo nó
    } else {
        ListaFlexivelSimples *aux = lista;
        while (aux->prox != NULL) {
            aux = aux->prox; // Percorre até o último nó
        }
        aux->prox = novoNo; // Insere o novo nó no final
    }
}

void inserir_inicio(ListaFlexivelSimples **lista, Show show)
{
    ListaFlexivelSimples *novoNo = (ListaFlexivelSimples *)malloc(sizeof(ListaFlexivelSimples));
    novoNo->show = show;
    novoNo->prox = *lista; // O novo nó aponta para o antigo primeiro nó
    *lista = novoNo;       // A lista agora aponta para o novo nó como primeiro
}

Show remover_fim(ListaFlexivelSimples **lista)
{
    if (*lista == NULL) {
    ...
    }else
    {
        ...
        if (aux->prox == NULL) { // Se só tem um elemento
            showRemovido = aux->show;
            free(aux);
            *lista = NULL; // Lista agora está vazia
        } else {
            while (aux->prox->prox != NULL) {
                aux = aux->prox; // Percorre até o penúltimo nó
            }
            showRemovido = aux->prox->show; // Guarda o valor do último nó
...
Show remover_inicio(ListaFlexivelSimples **lista)
{
    if (*lista == NULL) {
        printf("Lista vazia. Nada a remover.\n");
        ...
        ListaFlexivelSimples *aux = *lista;
        Show showRemovido = aux->show; // Guarda o valor do primeiro nó
        *lista = aux->prox; // O segundo nó agora é o primeiro
        free(aux); // Libera o antigo primeiro nó

        return showRemovido;
    }
}

void inserir_posicao(ListaFlexivelSimples **lista, Show show, int posicao)
{
    if (posicao < 0) {
        printf("Posição inválida. Deve ser >= 0.\n");
        return;
    }
    ...
    ListaFlexivelSimples *aux = *lista;
    for (int i = 0; i < posicao - 1 && aux != NULL; i++) {
        aux = aux->prox; // Percorre até a posição anterior
    }

    if (aux == NULL) {
        printf("Posição %d fora dos limites da lista. Inserindo no final.\n", posicao);
        inserir_fim(*lista, show);
    } else {
        novoNo->prox = aux->prox; // O novo nó aponta para o próximo nó
        aux->prox = novoNo;       // O nó anterior agora aponta para o novo nó
    }
}

Show remover_posicao(ListaFlexivelSimples **lista, int posicao)
{
    if (*lista == NULL) {
        printf("Lista vazia. Nada a remover.\n");
        Show vazio = {0}; // Retorna um show vazio
    ...
    if (posicao == 0) {
        showRemovido = aux->show; // Guarda o valor do primeiro nó
        *lista = aux->prox; // O segundo nó agora é o primeiro
        free(aux); // Libera o antigo primeiro nó
    ...
    if (aux == NULL || aux->prox == NULL) {
        printf("Posição %d fora dos limites da lista. Nada a remover.\n", posicao);
        Show vazio = {0}; // Retorna um show vazio
        return vazio;
    } else {
        ListaFlexivelSimples *noRemover = aux->prox;
        showRemovido = noRemover->show; // Guarda o valor do nó a ser removido
        aux->prox = noRemover->prox; // O nó anterior agora aponta para o próximo do nó removido
        free(noRemover); // Libera o nó
...
void imprimir_lista_flexivel_simples(ListaFlexivelSimples *lista)
{
    ListaFlexivelSimples *aux = lista;
    while (aux != NULL) {
...
void popular_lista_flexivel_simples(ListaFlexivelSimples **lista, Show *shows, int numShows)
{
    int numcomando = 0;
    char comando[200];
    scanf("%d", &numcomando);
    ...
    while (numcomando > 0) {
        ...
        char *partes[3]; // Para armazenar as partes do comando
        int num_partes = 0;
        char *token = strtok(comando, " ");

        while (token != NULL && num_partes < 3) {
        ...
        // Determina idShow e posicao com base na operacao
        ...
        if (strcmp(operacao, "I*") == 0) {
            inserir_posicao(lista, showEncontrado, posicao);
            // printf("(I) %s na posição %d\n", showEncontrado.title, posicao); // Imprime o título do show inserido
        } else if (strcmp(operacao, "II") == 0) {
            inserir_inicio(lista, showEncontrado); // Insere o show no início
            // printf("(I) %s no início\n", showEncontrado.title); // Imprime o título do show inserido
        } else if (strcmp(operacao, "IF") == 0) {
            inserir_fim(*lista, showEncontrado); // Insere o show no final
            // printf("(I) %s no final\n", showEncontrado.title); // Imprime o título do show inserido
        } else if (strcmp(operacao, "RI") == 0) {
            Show showRemovidoInicio = remover_inicio(lista); // Remove do início
            if (showRemovidoInicio.showId[0] != '\0') { // Verifica se não é um show vazio
                printf("(R) %s\n", showRemovidoInicio.title); // Imprime o título do show removido
            }
        } else if (strcmp(operacao, "RF") == 0) {
            Show showRemovidoFim = remover_fim(lista); // Remove do final
            if (showRemovidoFim.showId[0] != '\0') { // Verifica se não é um show vazio
                printf("(R) %s\n", showRemovidoFim.title); // Imprime o título do show removido
            }
        } else if (strcmp(operacao, "R*") == 0) {
            posicao = atoi(partes[1]); // Posição para remover
            Show showRemovido = remover_posicao(lista, posicao); // Remove da posição especificada
...
int main() {
    ...
    ListaFlexivelSimples *lista = NULL;
    inicializarLista_populada(&lista, showsFiltrados, quantidadeFiltrados);
    popular_lista_flexivel_simples(&lista, shows, total);

    // imprimir a lista flexível
    imprimir_lista_flexivel_simples(lista);
