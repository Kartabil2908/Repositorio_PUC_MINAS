...
void inserir_fila_circular(FilaCircular *fila, Show show)
{
    if (fila->tamanho == fila->capacidade) {
        // fila cheia — remover silenciosamente
    ...
    for (int i = 0; i < fila->tamanho; i++) {
        soma += fila->elementos[idx].releaseYear;
        idx = (idx + 1) % fila->capacidade;
    }
    printf("[Media] %d\n", (int)floor((double)soma / fila->tamanho));
}
...
Show *remover_fila_circular(FilaCircular *fila)
{
    ...
    Show *removido = (Show *)malloc(sizeof(Show));
    if (removido == NULL) {
        perror("Erro ao alocar memória");
        exit(1);
    }

    *removido = clonarShow(fila->elementos[fila->inicio]);
    fila->inicio = (fila->inicio + 1) % fila->capacidade;
    fila->tamanho--;
...
void imprimir_fila_circular(FilaCircular *fila)
{
    ...
    for (int i = 0; i < fila->tamanho; i++) {
        printf("[%d] ", i);
        imprimir_show(fila->elementos[idx]);
        idx = (idx + 1) % fila->capacidade;
    }
}
